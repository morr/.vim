...
if !exists('g:javascriptcomplete_library')
  let g:javascriptcomplete_library = ''
endif
...
function! javascriptcomplete#CompleteJS(findstart, base)
  ...
  if a:findstart
    ...
    if s:shortcontext =~ '\.$'
      " jQuery block
      if g:javascriptcomplete_library == 'jQuery'
        if match(s:shortcontext, '\(.*\)[') != -1
          let object_type = 'DOMElement'
        elseif match(s:shortcontext, '\(.*\)') != -1
          let s:shortcontext = substitute(s:shortcontext, '(.*', '.', '')
        end
      end

      let object.name = matchstr(s:shortcontext, '\zs\k\+\ze\(\[.\{-}\]\)\?\.$')
      " scope contains path+name
      let object.scope = substitute(s:shortcontext, '\.$', '', '')
      " prefix contains only path
      let object.prefix = substitute(substitute(object.scope, object.name.'$', '', ''), '\.$', '' ,'')
      let object.type = object_type

      if object.type == 'undefined'
        call s:FindObjectTypeInBuffers(object)
        if object.type == 'undefined'
          let object = s:FindObjectTypeInTags(object)
        endif
      endif
      "echoe 'name: '.object.name.' scope:'.object.scope.' prefix:'.object.prefix.' type:'.object.type
...
function! s:FindObjectTypeInTags(object)
  ...
  "echoe object_type
  "echoe 'name:'.a:object.name.', scope:'.a:object.scope.', prefix:'.a:object.prefix.', type:'.a:object.type
  ...
  " if object is not found but its scope contains '.' then lets try to reduce it
  if a:object.type == 'undefined' && match(a:object.scope, '\.') != -1
    let reduced_object = {}
    let reduced_object.name = substitute(a:object.scope, '\..*$', '', '')
    let reduced_object.scope = reduced_object.name
    let reduced_object.prefix = ''
    "let reduced_object.name = a:object.prefix
    "let reduced_object.scope = substitute(a:object.scope, "\.".a:object.name."$", "", "")
    "let reduced_object.prefix = substitute(substitute(reduced_object.scope, reduced_object.name.'$', '', ''), '\.$', '' ,'')
    let reduced_object_original_name = reduced_object.name
    let reduced_object.type = 'undefined'
    let reduced_object = s:FindObjectTypeInTags(reduced_object)
    if reduced_object.type != 'undefined'
      let a:object.prefix = substitute(substitute(a:object.prefix, reduced_object_original_name.'\(\.\|$\)', reduced_object.name.'.', ''), '\.$', '' ,'')
      let a:object.scope = substitute(a:object.scope, reduced_object_original_name.'\(\.\|$\)', reduced_object.name.'.', '')
      if reduced_object.scope != ""
        let a:object.prefix = reduced_object.scope.'.'.a:object.prefix
        let a:object.scope = reduced_object.scope.'.'.a:object.scope
      end
      "echoe 'new_object_after_scope_reduce:'.a:object.name.', scope:'.a:object.scope.', prefix:'.a:object.prefix.', type:'.a:object.type
      return s:FindObjectTypeInTags(a:object)
    end
  end
  ...
  if a:object.type != 'undefined' && !s:IsBuildInType(a:object.type)
    let a:object.scope = substitute(a:object.scope, '^'.escape(a:object.name, "$"), a:object.type, '')
    let a:object.scope = substitute(a:object.scope, '^\(.*\)::\w\+$', '\1', '')
    let a:object.name = substitute(a:object.type, '^.*::\(\w\+\)$', '\1', '')
    let a:object.prefix = substitute(substitute(a:object.scope, a:object.name.'$', '', ''), '\.$', '' ,'')
    let a:object.type = 'undefined'
    "echoe 'new_object name:'.a:object.name.', scope:'.a:object.scope.', prefix:'.a:object.prefix.', type:'.a:object.type
    return s:FindObjectTypeInTags(a:object)
  else
    return a:object
  endif
